...
def download_t86_csv(target_date):
    ...
    try:
        ...
        # Grok 核心逻辑：如果是空資料、網頁錯誤或查詢無資料，明確回傳 SKIPPED 讓程式往前走
...
with st.sidebar:
    ...
    if st.button("🔄 自動續傳更新", type="primary", use_container_width=True):
        with st.container():
            ...
            # 起始點：如果有舊資料就從下一天開始，沒有就從 4/27 開始
            ...
            while curr <= today:
                status_text.text(f"⏳ 正在檢查日期: {curr}")
                if is_trading_day(curr):
                    ...
                    if isinstance(day_df, pd.DataFrame) and not day_df.empty:
                        ...
                        curr += timedelta(days=1)  # 成功，前進
                    elif day_df == "SKIPPED":
                        st.toast(f"ℹ️ {curr} 證交所無資料，自動跳過。")
                        curr += timedelta(days=1)  # 無資料，聰明跳過前進下一天！
                    else:
                        st.toast(f"⚠️ {curr} 連線稍慢，等待重試...")
                        time.sleep(3)
                        # 真正的錯誤才留守重試，避免網站塞車
                else:
                    curr += timedelta(days=1)  # 假日自動前進
