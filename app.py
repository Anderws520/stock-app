...
# 🚀 修正後的下載函數：改用 JSON 介面，避開 CSV 格式或 Big5 編碼異動導致的解析失敗
def force_download(target_date):
    date_str = target_date.strftime('%Y%m%d')
    # 改用 response=json，並加上時間戳參數避免伺服器快取
    url = f"https://www.twse.com.tw/fund/T86?response=json&date={date_str}&selectType=ALLBUT0999&_={int(time.time()*1000)}"
    ...
    try:
        ...
        if not fields or not raw_data:
            return None
            
        # 直接由 JSON 轉為 DataFrame，無需處理複雜的字串切割
...
with st.sidebar:
    ...
    if st.button("🔄 自動更新資料（斷點續傳）", type="primary", use_container_width=True):
        with st.spinner("正在執行斷點續傳更新..."):
            ...
            st.success(f"✅ 斷點續傳完成！本次共更新 {updated} 天資料")
            time.sleep(1)
            st.rerun()
