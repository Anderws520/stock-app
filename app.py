...
st.set_page_config(page_title="台股法人工具 - 除錯版", layout="wide")
st.title("🟢 台股三大法人買超工具（除錯版）")
st.markdown("**專門解決無法下載資料的問題**")
...
def download_t86(date):
    ...
    try:
        ...
        st.caption(f"HTTP 狀態碼: {resp.status_code}")
        
        if resp.status_code != 200:
            st.error(f"下載失敗，狀態碼 {resp.status_code}")
            return None
        ...
        if start_idx is None:
            st.error("找不到『證券代號』標頭")
            st.text_area("回應內容預覽", resp.text[:800], height=200)
            return None
...
# ====================== 自動更新 ======================
if st.button("🔄 開始/繼續 自動更新資料（斷點續傳）", type="primary"):
    with st.spinner("正在執行斷點續傳..."):
        ...
        while target <= today and updated < 30:
            if is_trading_day(target):
                ...
                if new_df is not None and not new_df.empty:
                    ...
                    updated += 1
            progress.progress(min(updated / 25, 1.0))
            target += timedelta(days=1)
            time.sleep(6)
        
        st.success(f"更新完成！本次更新 {updated} 天")
